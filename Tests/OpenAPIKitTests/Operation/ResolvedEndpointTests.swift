...
final class ResolvedEndpointTests: XCTestCase {
    func test_takesOnPathAndOperationProperties() throws {
        ...
        XCTAssertEqual(endpoints.first?.operationSummary, "endpointSummary")
        XCTAssertEqual(endpoints.first?.operationDescription, "endpointDescription")
        XCTAssertEqual(endpoints.first?.operationVendorExtensions["test"]?.value as? String, "endpoint")
