...
final class ContentTypeTests: XCTestCase {
    func test_badContentType() {
        let bad = ContentType(rawValue: "not a content type")
        XCTAssertEqual(
    ...
    func test_contentTypeStringReflexivity() {
        ...
        for type in types {
            XCTAssertEqual(type, ContentType(rawValue: type.rawValue))
        }
    }

    func test_goodParam() {
        let type = ContentType.init(rawValue: "text/html; charset=utf8")
        XCTAssertEqual(type?.warnings.count, 0)
        XCTAssertEqual(type?.rawValue, "text/html; charset=utf8")
    }

    func test_multipleParams() {
        let type = ContentType.init(rawValue: "my/type; some=thing; another=else")
        XCTAssertEqual(type?.warnings.count, 0)
