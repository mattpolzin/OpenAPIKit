...
//      async let (newExamples, c2) = oldExamples.externallyDereferenced(with: loader)
//      async let (newEncoding, c3) = oldEncoding.externallyDereferenced(with: loader)

      var newContent = self
      var newComponents = try await c1

      newContent.schema = try await newSchema
//      newContext.examples = try await newExamples
//      newComponents.merge(c2)
//      newContext.encoding = try await newEncoding
//      newComponents.merge(c3)

       // TOOD: need to locally dereference the examples, and encoding here.
#warning("need to locally dereference the examples, and encoding here.")
